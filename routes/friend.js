...
//                  res.json(matches.slice(offset, offset + limit));
              ...
              res.json([{_id:"error",username:"No matches found."}]);
            }
        ...
        discoverfriends: function (userid, offset, limit, sort) {
          ...
          var p_id = BSON.ObjectID.createFromHexString(req.session.currentUser._id);
          users.findOne({"_id": {$in: [p_id]}},'friends friendspending',function(err,friendslists){
            var friends = [p_id];
            if(typeof friendslists.friends != "undefined") {
              _.each(friendslists.friends, function (friendid) {
            ...
            users.find({"_id": {$nin: _.uniq(friends,false)}}, 'username', function (err, newpeople) {
              // Have list of users, plus logged in user.
              if(newpeople.length){
                var everyone = [p_id.toString()];
...
//                    res.json(newpeople.slice(offset,offset+limit));
                ...
                res.json([{_id:"error",username:"You're friends with everyone already"}]);
              }
            });
          });
        },
        ...
        addfriend: function (friendid) {
          ...
          users.update(
            { "_id": {$in: [p_id]}, "friends": {$in:[f_id.toString()]}, "friendspending": {$in: [f_id.toString()]} },
            {$pullAll:{friendspending: [p_id.toString(), f_id.toString()]}, $addToSet: {friends: f_id.toString()}},
            {upsert: false, multi: true}
          );
          users.update(
            { "_id": {$in: [f_id]}, "friends": {$in:[p_id.toString()]}, "friendspending": {$in: [p_id.toString()]} },
            {$pullAll:{friendspending: [p_id.toString(), f_id.toString()]}, $addToSet: {friends: p_id.toString()}},
            {upsert: false, multi: true}
          );
          // Poor error checking, but not yet sure how to handle verifying both of the above steps before returning a value in async
